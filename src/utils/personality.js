...
export const renderNoFoodDetectedReply = ({ imageSubject, imageCaption } = {}) => {
  ...
  if (/ไก่|chicken/i.test(detail)) {
    if (/กาแฟ|coffee|birdy|กระป๋อง/i.test(detail)) {
      return [
        "อันนี้ไก่ถือกาแฟนะ 555++",
        "แปะขอแปะไว้ก่อน ยังไม่นับเป็นมื้อให้จ้า",
        "ส่งรูปอาหารจริงมา เดี๋ยวแปะจัดให้ 🍽️",
      ].join("\n");
    }

    return [
      "อันนี้ไก่นะ 555+",
      "ยังไม่ใช่มื้อที่แปะนับแคลให้ได้จ้า",
      "ส่งรูปอาหารมา เดี๋ยวแปะจัดให้ 🍽️",
    ].join("\n");
  }

  if (/แมว|cat/i.test(detail)) {
    return [
      "เอ้า อันนี้แมวนี่นา 555+",
  ...
  if (/หมา|สุนัข|dog/i.test(detail)) {
    return [
      "อันนี้น้องหมานะ 555+",
      "แปะขอแปะไว้ก่อน ยังไม่ใช่มื้อข้าว",
      "ส่งรูปจานที่กินมา เดี๋ยวแปะจัดให้ 🍽️",
    ].join("\n");
  }
  ...
  if (/จอ|คอม|หน้าจอ|screenshot|screen/i.test(detail)) {
    return [
      "อันนี้เหมือนรูปหน้าจอนะ 555",
      "แปะขอแปะไว้ก่อน ยังนับแคลไม่ได้จ้า",
      "ส่งรูปอาหารจริงมา เดี๋ยวแปะจัดให้ 🍽️",
    ].join("\n");
  }
  ...
  if (subject && !/อาหาร|food|meal/i.test(subject)) {
    return [
      `อันนี้เหมือนรูป${subject}นะ 555+`,
      "ยังไม่ใช่อาหารที่แปะนับแคลได้จ้า",
      "ส่งรูปจานที่กินมา เดี๋ยวแปะจัดให้ 🍽️",
    ].join("\n");
  }

  return [
    "อันนี้ยังไม่ใช่อาหารนะ 555",
    "แปะขอแปะไว้ก่อน นับแคลให้ไม่ถนัดจ้า",
    "ส่งรูปจานที่กินมา เดี๋ยวแปะจัดให้ 🍽️",
  ].join("\n");
};
