...
const inferTitleEmoji = (name = "", category = "") => {
  const text = `${name} ${category}`.toLowerCase();
  if (/sugar|sweet|หวาน|น้ำตาล|ชาไทย|ชาตรามือ|dessert|caramel|cream|ไซรัป/.test(text)) return "🧋";
  if (/fried|ทอด|กรอบ|ชีส|oil|crunchy|หม้อทอด|มัน/.test(text)) return "🍟";
  if (/carb|แป้ง|ข้าว|เส้น|noodle|rice/.test(text)) return "🍚";
  if (/protein|โปรตีน|อกไก่|เวย์|lean|macro|กล้าม/.test(text)) return "💪";
  if (/calorie|deficit|portion|discipline|คุม|ร่างทอง|clean|healthy|balanced|zen/.test(text)) return "✨";
  if (/midnight|night|ดึก|ตีหนึ่ง|7-eleven|ตู้เย็น|ot/.test(text)) return "🌙";
  if (/cheat|buffet|bulk|อิ่ม|แคลทะลุ|full course|enjoy/.test(text)) return "🔥";
  if (/comfort|ฮีล|เยียวยา|soul|เหนื่อย|support|ชาไทย/.test(text)) return "🫶";
  if (/wizard|chosen|legendary|god|emperor|rare/.test(text)) return "👑";
  return "✨";
};

const title = (name, rarity = "Common", vibe = "", emoji = "") => ({
  name,
  rarity,
  emoji: emoji || inferTitleEmoji(name, vibe),
...
const TITLE_BANK = {
  ...
  fried: [
    ...
    title("น้ำมันคือพลังงานชีวิต", "Rare", "วันนี้ทอดมีซีน", "🛢️"),
    ...
    title("Oil Era Activated", "Epic", "ยุคน้ำมันเริ่มแล้ว", "🛢️"),
  ],
  carb: [
  ...
  lowProtein: [
    title("โปรตีนหายเข้ากลีบเมฆ", "Common", "แปะหาแล้ว ยังไม่เจอ", "☁️"),
  ...
  proteinGood: [
    ...
    title("กินเหมือนคนจะฟิตจริง", "Rare", "แปะเริ่มเชื่อแล้วนะ", "🏋️"),
    title("Macro Respecter", "Epic", "เคารพมาโครแบบคนมีวินัย", "📊"),
    title("Lean Machine", "Legendary", "ทรงเริ่มคม", "⚙️"),
  ...
  over: [
    ...
    title("Buffet Energy", "Epic", "พลังบุฟเฟต์เข้าสิง", "🍽️"),
  ],
  balanced: [
    title("สมดุลเกินมนุษย์", "Epic", "แปะเริ่มสงสัยว่าซ้อมมา", "⚖️"),
    title("Aura คนดูแลตัวเอง", "Rare", "ดูแลตัวเองแบบไม่เว่อร์", "✨"),
    title("Healthy แต่ยังอร่อย", "Common", "อันนี้แหละทางสายกลาง", "🥗"),
    title("Balanced Lifestyle Holder", "Epic", "ถือสมดุลไว้แน่น", "⚖️"),
  ...
  rare: [
    ...
    title("The Chosen Meal", "Secret", "มื้อนี้ถูกเลือกแล้ว", "🗝️"),
