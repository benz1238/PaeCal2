...
const buildEatingGuiltReply = (text = "") => {
  ...
  if (/(น้ำหนักขึ้น|ขึ้น\s*\d+\s*(โล|กก|kg)|อ้วนแน่|วันนี้อ้วนแน่|บวมแน่)/i.test(value)) {
    const weightReplies = [
      "ใจเย็นก่อนน้า 555+\nเลขตาชั่งมันแกว่งได้ อย่าไปซี\nพน.เริ่มใหม่ แปะจัดให้",
      "ตาชั่งวันนี้แอบตึงนิดนึง 555\nแต่มันขึ้นลงได้หลายอย่างอยู่\nวันนี้ตั้งหลักก่อน พน.คุมเกมใหม่",
      "แปะขอแปะไว้ก่อนนะ 👀\nคำว่าอ้วนอย่าเพิ่งโยนใส่ตัวเอง\nพน.เบาลงนิด แค่นี้ยังเอาอยู่",
      "เลขขึ้นนิดหน่อยยังไม่ใช่จบเกมนะ 555++\nอย่าไปซีมาก\nมื้อต่อไปเบาลงนิดเดียว แปะว่าไหว"
    ];
  ...
  if (/(จุก|แน่น|อิ่มจุก|กินอย่างหนัก|กินหนักมาก|กินไปอย่างหนัก)/i.test(value)) {
    const heavyReplies = [
      "โอเค วันนี้จัดเต็มจริง 555+\nจุกแล้วพักก่อน ไม่ต้องเติมต่อ\nพน.ค่อยรีเซ็ตใหม่ ชิล ๆ",
      "วันนี้คือกินแบบเปิดโหมดบอส 555\nแต่ไม่เป็นไร จบมื้อนี้พอ\nพน.แปะจัดให้ใหม่",
      "เอ้า จุกแล้วก็แปะไว้ก่อนนะ 😂\nอย่าไปซีเกิน\nคืนนี้พักท้อง พน.ค่อยเอาใหม่",
      "มื้อนี้เสียงดังนิดนึง 555\nแต่เกมยังไม่แตก\nพน.เบาลงหน่อย แปะว่าเอาอยู่"
    ];
  ...
  if (/(กินเละ|กินพัง|หลุดหนัก|หลุดยับ|พังแน่|กินจนรู้สึกผิด)/i.test(value)) {
    const guiltReplies = [
      "เอ้า อย่าเพิ่งแพนิค 555\nวันนี้หลุดได้ ไม่ได้แปลว่าพัง\nพน.เริ่มใหม่ แปะจัดให้",
      "แปะเห็นแล้ว 555+\nแต่ไม่ต้องด่าตัวเองแรงขนาดนั้น\nมื้อต่อไปคุมเกมนิดเดียวพอ",
      "วันนี้แปะไว้ก่อนนะ 👀\nไม่ต้องซี มื้อเดียวไม่ใช่ชีวิตทั้งวีค\nพน.ค่อยกลับมาใหม่",
      "ยังไม่พัง ยังไม่จบเกม 555\nแค่วันนี้กินแรงไปหน่อย\nพน.เอาใหม่ เดี๋ยวแปะดูให้"
    ];
  ...
  const replies = [
    ...
    "ไม่ต้องตีตัวเองก่อนน้า 👀\nวันนี้จัดหนักได้ พรุ่งนี้ค่อยดึงกลับ\nชิล ๆ แต่ไม่ปล่อยยาวพอ",
    "โอเค วันนี้กินเพลินไปนิด 555+\nไม่เป็นไร แปะไว้ก่อน\nมื้อต่อไปเบาลงนิดนึงก็พอ",
    "กินเยอะได้ แต่อย่าไปซีเกิน 555\nแค่หยุดเติมก่อน แล้วพน.เริ่มใหม่\nแปะจัดให้ต่อเอง"
  ];
