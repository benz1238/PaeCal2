...
const buildEatingGuiltReply = (text = "") => {
  ...
  if (/(น้ำหนักขึ้น|ขึ้น\s*\d+\s*(โล|กก|kg)|อ้วนแน่|วันนี้อ้วนแน่|บวมแน่)/i.test(value)) {
    return pickReply([
      "ใจเย็นก่อนน้า 😅\nเลขตาชั่งมันแกว่งได้ ไม่ต้องซีขนาดนั้น\nพน.เริ่มใหม่ คุมเบา ๆ ก็กลับมาได้",
      "อย่าเพิ่งแพนิคกับเลขบนตาชั่ง 👀\nวีคนึงขึ้นลงได้หลายอย่างอยู่\nวันนี้พักก่อน พน.ค่อยคุมเกมใหม่",
      "แปะขอเบรกคำว่าอ้วนก่อนนะ 😅\nช่วงนี้อาจกินหนักไปนิด แต่ยังไม่พัง\nพน.เอาใหม่แบบชิล ๆ จ้า",
      "เลขขึ้นนิดหน่อยยังไม่ใช่ final boss นะ 😂\nอย่าไปซีมาก\nมื้อต่อไปเบาลงนิดเดียว แปะว่าเอาอยู่"
    ]);
  }

  if (/(จุก|แน่น|อิ่มจุก|กินอย่างหนัก|กินหนักมาก|กินไปอย่างหนัก)/i.test(value)) {
    return pickReply([
      "โอเค วันนี้จัดเต็มจริง 😂\nจุกก็พักก่อน ไม่ต้องฝืนต่อ\nพน.ค่อยเริ่มใหม่ แปะว่าไม่เสียหาย",
      "วันนี้คือกินแบบตัวแม่ไปนิดนึง 👀\nไม่ต้องแพนิค แค่หยุดเติมก่อนพอ\nมื้อต่อไปเบา ๆ เดี๋ยวบาลานซ์กลับ",
      "เอ้า จุกแล้วก็จบมื้อนี้ก่อนนะ 😂\nอย่าไปซีเกิน\nคืนนี้พักท้อง พน.ค่อยเอาใหม่",
      "เข้าใจ ฟีลหลุดแล้วไหลยาวอะเนอะ 😅\nแต่ไม่เป็นไร ยังกลับได้\nมื้อต่อไปแปะช่วยคุมให้"
    ]);
  }

  if (/(กินเยอะ|เยอะมาก|กินเกิน|หลุดหนัก|หลุดยับ|กินเละ|กินพัง|พังแน่)/i.test(value)) {
    return pickReply([
      "เอ้า ใจเย็นก่อน 😂\nกินเยอะวันนึงไม่ได้ทำให้พังทันที\nพน.เริ่มใหม่ ชิล ๆ จ้า",
      "ไม่ต้องตีตัวเองก่อนน้า 👀\nวันนี้อาจหลุดนิด แต่ยังไม่จบเกม\nมื้อต่อไปคุมเบา ๆ ก็พอ",
      "โอเค วันนี้อาจจะ all in ไปหน่อย 😂\nแต่ไม่ต้องเครียดนะ\nพักของทอดหวานรอบถัดไปนิดนึง เริ่ดละ",
      "หลุดได้ ไม่ได้แปลว่าเละถาวร 😄\nอย่าไปซีมาก\nเดี๋ยวมื้อต่อไปแปะช่วยดึงกลับ"
    ]);
  }

  return pickReply([
    "เอ้า อย่าเพิ่งแพนิค 😂\nมื้อเดียวไม่ทำให้พังหรอก\nพน.เริ่มใหม่ได้ แปะว่าเอาอยู่",
    "ไม่ต้องด่าตัวเองแรงขนาดนั้นน้า 👀\nวันนี้หลุดได้ พน.ค่อยกลับมา\nชิล ๆ แต่ไม่ปล่อยยาวพอ",
    "โอเค แปะรับทราบความจุก 😅\nวันนี้พอแค่นี้ก่อน\nมื้อต่อไปเบาลงนิดนึง จบสวย",
    "ยังไม่พัง ยังไม่จบเกม 😂\nแค่วันนี้กินแรงไปหน่อย\nพน.เอาใหม่ เดี๋ยวแปะดูให้"
  ]);
};
